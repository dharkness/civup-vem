...
function RefreshCityBanner(cityBanner, iActiveTeam, iActivePlayer)
	...
	if city ~= nil then	
		...
		if bShowBasicHelp then
			cityTip = cityTip .. Locale.ConvertTextKey("TXT_KEY_CITY_ENTER_CITY_SCREEN")
		end
...
-- Cope: [ SHOWPOPFALL ] Show number of turns until pop falls by 1
-- Weapon
...
--Cope: [ SHOWPOPFALL ] Make growth meter red if city is starving
			...
			local iFoodPerTurn = city:IsFoodProduction() and 0 or Round(City_GetYieldRate(city, YieldTypes.YIELD_FOOD), 1);
...
-- Cope: [ CULTUREPOP ] Update the culture growth meter
...
-------------
	...
	if city ~= nil and not isActivePlayerCity then
		-- Players have met
		if (Teams[Game.GetActiveTeam()]:IsHasMet(player:GetTeam())) then
			
			local strToolTip = ""
			if bShowBasicHelp then
				strToolTip = strToolTip .. Locale.ConvertTextKey("TXT_KEY_TALK_TO_PLAYER");
			else
				strToolTip = strToolTip .. Locale.ConvertTextKey("TXT_KEY_CITY_OF", Players[city:GetOwner()]:GetCivilizationAdjectiveKey(), city:GetName());				
			end
