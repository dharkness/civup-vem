...
include ( "EPM_Common Plot Helpers.lua" );
include( "AlpacaUtils" );
	...
	for i = 0, numUnits do
		...
		if (unit ~= nil and not unit:IsInvisible(iActiveTeam, bIsDebug)) then
			...
			if (OptionsManager:IsDebugMode()) then
				strStatus = strStatus .. " ("..tostring(unit:GetOwner()).." - " .. tostring(unit:GetID()) .. ")";
			end
			...
			if iActiveTeam == unitTeam and unit:IsCombatUnit() or unit:GetDomainType() == DomainTypes.DOMAIN_AIR then
				strStatus = strStatus .. Locale.ConvertTextKey("TXT_KEY_HOVERINFO_EXPERIENCE", GetExperienceStored(unit), GetExperienceNeeded(unit)) .. " ";
			end
