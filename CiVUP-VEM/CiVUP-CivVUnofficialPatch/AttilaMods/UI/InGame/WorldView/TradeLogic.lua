...
function OnOpenPlayerDealScreen( iOtherPlayer )
	...
	if( g_iUs == g_iThem ) then
	    print( "ERROR: OpenPlayerDealScreen called with local pPlayer" );
    end
...
function OpenDealReview()
	...
	if( g_iUs == g_iThem ) then
	    print( "ERROR: OpenDealReview called with local pPlayer" );
    end
...
function ResetDisplay()
	...
	-- Us
	local iGoldPerTurn = Player_GetYieldRate(g_pUs, YieldTypes.YIELD_GOLD);
    ...
    -- Them
	iGoldPerTurn = Player_GetYieldRate(g_pThem, YieldTypes.YIELD_GOLD);
    strGoldString = iGoldPerTurn .. " " .. Locale.ConvertTextKey("TXT_KEY_DIPLO_GOLD_PER_TURN");
       ...
       --print("Adding trade item to active deal: " .. itemType);
        ...
        elseif( TradeableItems.TRADE_ITEM_GOLD_PER_TURN == itemType ) then
            ...
            if( bFromUs ) then
				...
				strTooltip = Locale.ConvertTextKey( "TXT_KEY_DIPLO_CURRENT_GPT", Player_GetYieldRate(g_pUs, YieldTypes.YIELD_GOLD) - data1 );
				...
				strTooltip = Locale.ConvertTextKey( "TXT_KEY_DIPLO_CURRENT_GPT", Player_GetYieldRate(g_pThem, YieldTypes.YIELD_GOLD) - data1 );
...
function PocketGoldPerTurnHandler( isUs )
    ...
    if( isUs == 1 ) then
		...
		if (iGoldPerTurn > Player_GetYieldRate(g_pUs, YieldTypes.YIELD_GOLD)) then
			iGoldPerTurn = Player_GetYieldRate(g_pUs, YieldTypes.YIELD_GOLD);
		end
		
        g_Deal:AddGoldPerTurnTrade( g_iUs, iGoldPerTurn, g_iDealDuration );
    else
		
		if (iGoldPerTurn > Player_GetYieldRate(g_pThem, YieldTypes.YIELD_GOLD)) then
			iGoldPerTurn = Player_GetYieldRate(g_pThem, YieldTypes.YIELD_GOLD);
		end
...
function ChangeGoldPerTurnAmount( string, control )
    ...
    if( control:GetVoid1() == 1 ) then
		...
		local strTooltip = Locale.ConvertTextKey( "TXT_KEY_DIPLO_CURRENT_GPT", Player_GetYieldRate(g_pUs, YieldTypes.YIELD_GOLD) - iGoldPerTurn );
		Controls.UsTableGoldPerTurn:SetToolTipString( strTooltip );
    ...
    -- GPT from them
		...
		local strTooltip = Locale.ConvertTextKey( "TXT_KEY_DIPLO_CURRENT_GPT", Player_GetYieldRate(g_pThem, YieldTypes.YIELD_GOLD) - iGoldPerTurn );
		Controls.ThemTableGoldPerTurn:SetToolTipString( strTooltip );
