...
function TechSelected( eTech, iDiscover)
   	Network.SendResearch(eTech, iDiscover, false);
	ClosePopup();
end
...
function OnDisplay(popupInfo)
	...
	OpenAdvisorPopup(ButtonPopupTypes.BUTTONPOPUP_CHOOSETECH);
...
function OnTechPanelUpdated()
	...
	Game.SetAdvisorRecommenderTech( playerID );
  	...
  	local iDiscover = pPlayer:GetNumFreeTechs();
    ...
    if (not Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
 		for tech in GameInfo.Technologies() do
 			...
 			if pPlayer:CanResearch( techID ) then
				AddTechButton( tech, Player_GetYieldTurns(pPlayer, YieldTypes.YIELD_SCIENCE,  techID), iDiscover);
			end
		end
	end
...
function AddTechButton( tech, turnsLeft, iDiscover)
	...
	thisTechButtonInstance.FreeTechName:SetText( techName );
  	...
  	local researchTurnsLeft = math.ceil(Player_GetYieldTurns(pPlayer, YieldTypes.YIELD_SCIENCE, tech.ID));
 	local turnText = tostring( researchTurnsLeft ).." "..turnsString;

	if pPlayer:GetCurrentResearch() == tech.ID then -- the pPlayer is currently researching this one
		...
		if iDiscover > 0 then
			...
			-- update number of turns to research
 			if 	Player_GetYieldRate(pPlayer, YieldTypes.YIELD_SCIENCE) > 0 then
		...
		local researchTurnsLeft = Player_GetYieldTurns(pPlayer, YieldTypes.YIELD_SCIENCE,  techID);
		local currentResearchProgress = teamTechs:GetResearchProgress(techID) + pPlayer:GetOverflowResearch();
		local researchNeeded = Player_GetYieldNeeded(pPlayer, YieldTypes.YIELD_SCIENCE);
		local researchPerTurn = Player_GetYieldRate(pPlayer, YieldTypes.YIELD_SCIENCE);
 	...
 	elseif pPlayer:CanResearch( tech.ID ) then -- the pPlayer research this one right now if he wants
 		thisTechButtonInstance.CurrentlyResearching:SetHide( true );
 		-- deal with free tech
		if iDiscover > 0 then
 			thisTechButtonInstance.Available:SetHide( true );
