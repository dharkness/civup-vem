...
function UpdateCombatOddsUnitVsCity(pMyUnit, pCity)
	...
	local sizeY = Constrain(minRows, math.max(myRows, theirRows), maxRows) * rowWidth + rowBuffer;
	--log:Debug("myRows=%s constrained=%s width=%s buffer=%s", myRows, Constrain(minRows, math.max(myRows, theirRows), maxRows), Constrain(minRows, math.max(myRows, theirRows), maxRows) * rowWidth, rowBuffer)
				...
				if (pPlot:IsRevealed(iTeam, false)) then
					...
					if (pPlot:IsCity()) then
						...
						if (pTeam:IsAtWar(pCity:GetTeam()) or (UIManager:GetAlt() and pCity:GetOwner() ~= iTeam)) then
							...
							Controls.UnitName:SetText(pCity:GetName());
