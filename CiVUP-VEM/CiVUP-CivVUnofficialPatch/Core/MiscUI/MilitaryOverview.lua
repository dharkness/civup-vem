...
function UpdateScreen()
    ...
    if( not bInDeficit ) then
	    ...
	    Controls.SupplyDeficitPenaltyValue:SetText( Player_GetSupplyModifier(pPlayer, YieldTypes.YIELD_PRODUCTION, true) .. "%" );
    end
...
function ShowHideHandler( bIsHide, bInitState )
    ...
    if( not bInitState ) then
        if( not bIsHide ) then
			...
			OpenAdvisorPopup(ButtonPopupTypes.BUTTONPOPUP_MILITARY_OVERVIEW);
        	m_bSortReverse = not m_bSortReverse;
        	OnSort( m_SortMode );
        	Events.SerialEventGameMessagePopupShown(m_PopupInfo);
        else
            UI.decTurnTimerSemaphore();
            CloseAdvisorPopup(ButtonPopupTypes.BUTTONPOPUP_MILITARY_OVERVIEW);
