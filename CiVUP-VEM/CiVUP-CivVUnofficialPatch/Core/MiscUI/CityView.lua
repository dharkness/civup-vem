...
function AddBuildingButton( pCity, building )
	...
	if pCity:IsHasBuilding(buildingID) and building.IsVisible then
		...
		elseif (building.AdditionParent) then
			...
			for buildingInfo in GameInfo.Buildings(string.format("AdditionParent = '%s'", buildingClass)) do
				if pCity:IsHasBuilding(buildingInfo.ID) then
		...
		local numCopies = GetNumBuilding(pCity, buildingID)
		if numCopies > 1 then
...
function UpdateThisQueuedItem(pCity, queuedItemNumber, queueLength)
	...
	queuedOrderType, queuedData1, queuedData2, queuedSave, queuedRush = pCity:GetOrderFromQueue( queuedItemNumber-1 );
	local thisTable = nil
	...
	if thisTable then
		...
		Controls[controlTurns]:SetText(  Locale.ConvertTextKey("TXT_KEY_PRODUCTION_HELP_NUM_TURNS",City_GetYieldTurns( pCity, YieldTypes.YIELD_PRODUCTION, thisTable, queuedData1, queuedItemNumber-1)) );
...
function OnCityViewUpdate()
	...
	if (pCity ~= nil) then
		...
		local iTurnsToGrowth = City_GetYieldTurns(pCity, YieldTypes.YIELD_FOOD);
		...
		for building in GameInfo.Buildings() do
			...
			if buildingSC > 0 and (pCity:IsHasBuilding(buildingID)) then
				local buildingST = building.SpecialistType;
				local buildingFC = pCity:GetNumSpecialistsInBuilding(buildingID);
				...
				for i, v in ipairs(sortedList) do
					...
					for buildingAddition in GameInfo.Buildings() do
						...
						if buildingAddition.AdditionParent == buildingClass then
							if (pCity:IsHasBuilding(buildingAdditionID)) then
								local addition = GameInfo.Buildings[buildingAdditionID];
								AddBuildingButton( pCity, addition );
							end
						end
					end
				end
...
--		UpdateSpecialists(pCity);
		...
		-- display gold income
		local iGoldPerTurn = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_GOLD), 1);
		...
		if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
			...
			local iSciencePerTurn = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_SCIENCE), 1);
		...
		local iCulturePerTurn = City_GetYieldRate(pCity, YieldTypes.YIELD_CULTURE);
		...
		local cultureStored = City_GetYieldStored(pCity, YieldTypes.YIELD_CULTURE);
		local cultureNext = City_GetYieldNeeded(pCity, YieldTypes.YIELD_CULTURE);
		...
		local cityGrowth = math.ceil(City_GetYieldTurns(pCity, YieldTypes.YIELD_FOOD));
		...
		local iFoodPerTurn = Round(City_GetYieldRateTimes100(pCity, YieldTypes.YIELD_FOOD) / 100,1);
		...
		local iCurrentFood = City_GetYieldStored(pCity, YieldTypes.YIELD_FOOD);
		local iFoodNeeded = City_GetYieldNeeded(pCity, YieldTypes.YIELD_FOOD);
		local iFoodDiff = City_GetYieldRateTimes100(pCity, YieldTypes.YIELD_FOOD)/100;
...
function DoUpdateProductionInfo( bNoProduction )
	...
	local iStoredProduction = City_GetYieldStored(pCity, YieldTypes.YIELD_PRODUCTION);
	...
	local iProductionPerTurn = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_PRODUCTION), 1)
	...
	-- Turns left
	local productionTurnsLeft = City_GetYieldTurns(pCity, YieldTypes.YIELD_PRODUCTION);
	...
	local bGeneratingProduction = pCity:IsProductionProcess() or City_GetYieldRate(pCity, YieldTypes.YIELD_PRODUCTION) == 0;
	...
	local strProductionHelp = GetYieldTooltip(pCity, YieldTypes.YIELD_PRODUCTION);
...
function DoUpdateUpperLeftTooltips()
	...
	local strFoodToolTip = GetYieldTooltip(pCity, YieldTypes.YIELD_FOOD);
	Controls.FoodBox:SetToolTipString(strFoodToolTip);
	Controls.PopulationBox:SetToolTipString(strFoodToolTip);
	Controls.GoldBox:SetToolTipString(GetYieldTooltip(pCity, YieldTypes.YIELD_GOLD));
	Controls.ScienceBox:SetToolTipString(GetYieldTooltip(pCity, YieldTypes.YIELD_SCIENCE));
	Controls.CultureBox:SetToolTipString(GetYieldTooltip(pCity, YieldTypes.YIELD_CULTURE));
...
function OnProductionClick()
	...
	local popupInfo = {
		...
		Data2 = -1,
		Data3 = -1,
		Option1 = (productionQueueOpen and pCity:GetOrderQueueLength() > 0),
		Option2 = false;
	}
	Events.SerialEventGameMessagePopup(popupInfo);
...
function OnBuyPlotClick()
	...
	--Network.SendCityBuyPlot(pCity:GetID(), -1, -1);
	--UpdateBuyPlotButton();
end
...
function OnMouseOverHex( hexX, hexY )
	...
	if UI.GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_PURCHASE_PLOT then
		...
		-- Can buy this plot
		if (pCity:CanBuyPlotAt( hexX, hexY, true)) then
			local iPlotCost = pCity:GetBuyPlotCost( hexX, hexY );
