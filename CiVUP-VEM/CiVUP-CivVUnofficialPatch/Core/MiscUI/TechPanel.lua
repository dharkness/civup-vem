...
function OnTechPanelUpdated()
	...
	if (eCurrentTech ~= -1) then
		...
		local iCurrentResearchProgress = Player_GetYieldStored(pPlayer, YieldTypes.YIELD_SCIENCE,  eCurrentTech);
		local iResearchNeeded = Player_GetYieldNeeded(pPlayer, YieldTypes.YIELD_SCIENCE,  eCurrentTech);
		local iResearchPerTurn = Player_GetYieldRate(pPlayer, YieldTypes.YIELD_SCIENCE);
