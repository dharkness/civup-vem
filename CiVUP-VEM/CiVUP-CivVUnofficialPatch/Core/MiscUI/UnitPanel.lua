...
function UpdateUnitPortrait( unit )
    ...
    if unit:IsCombatUnit() or unit:GetDomainType() == DomainTypes.DOMAIN_AIR then
		...
		local iExperienceNeeded = GetExperienceNeeded(unit);
...
function OnInfoPaneDirty()
    ...
    if (unit ~= nil) then
		...
		-- Attempt to show currently selected pCity.
...
function TipHandler( control )
	...
	if (bBuild) then
		...
		do
			...
			iYieldChange = iYieldChange - pPlot:GetYield(iYield) --alpaca pPlot:CalculateYield(iYield);
