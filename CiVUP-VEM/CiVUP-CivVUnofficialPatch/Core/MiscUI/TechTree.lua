...
function RefreshDisplayOfSpecificTech( tech )
	...
	local thisTechButton = techButtons[techID];
 	...
 	local researchTurnsLeft = math.ceil(Player_GetYieldTurns(player, YieldTypes.YIELD_SCIENCE,   techID));
 	...
 	elseif player:GetCurrentResearch() == techID then -- the player is currently researching this one
		...
		if player:GetNumFreeTechs() > 0 then
			...
			-- update number of turns to research
 			if 	Player_GetYieldRate(player, YieldTypes.YIELD_SCIENCE) > 0 then
		...
		local researchTurnsLeft = Player_GetYieldTurns(player, YieldTypes.YIELD_SCIENCE,  techID);
		local currentResearchProgress = Player_GetYieldStored(player, YieldTypes.YIELD_SCIENCE);
		local researchNeeded = Player_GetYieldNeeded(player, YieldTypes.YIELD_SCIENCE);
		local researchPerTurn = Player_GetYieldRate(player, YieldTypes.YIELD_SCIENCE);
