...
function UpdateDisplay()
    ...
    for pCity in pPlayer:Cities() do
		...
		if (instance.GrowthBar) then
			...
			local iFoodPerTurn = pCity:IsFoodProduction() and 0 or Round(City_GetYieldRate(pCity, YieldTypes.YIELD_FOOD));
		...
		sortEntry.Food = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_FOOD));
        instance.Food:SetText( sortEntry.Food );
       
		sortEntry.Production = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_PRODUCTION));
        instance.Production:SetText( sortEntry.Production );
        
        sortEntry.Science = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_SCIENCE));
        instance.Science:SetText( sortEntry.Science );
        
        sortEntry.Gold = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_GOLD));
        instance.Gold:SetText( sortEntry.Gold );
        
        sortEntry.Culture = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_CULTURE));
...
function UpdateGPT()
    ...
    for pCity in pPlayer:Cities() do
        ...
        local CityIncome = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_GOLD));
	...
	local iTotalUnitMaintenance = Player_CalculateUnitCost(pPlayer);
...
function ProductionDetails( pCity, instance )
	...
	if (instance.ProductionBar) then
		...
		local iProductionPerTurn = Round(City_GetYieldRate(pCity, YieldTypes.YIELD_PRODUCTION));
