...
function UpdateDisplay()
	...
	local culturePerTurn = Round(Player_GetYieldRate(player, YieldTypes.YIELD_CULTURE));
    ...
    local szText = Locale.ConvertTextKey("TXT_KEY_NEXT_POLICY_COST_LABEL", Player_GetYieldNeeded(player, YieldTypes.YIELD_CULTURE));
    Controls.NextCost:SetText(szText);
    
    szText = Locale.ConvertTextKey("TXT_KEY_CURRENT_CULTURE_LABEL", Player_GetYieldStored(player, YieldTypes.YIELD_CULTURE));
    ...
    local iTurns;
    local iCultureNeeded = Player_GetYieldNeeded(player, YieldTypes.YIELD_CULTURE) - Player_GetYieldStored(player, YieldTypes.YIELD_CULTURE);
	...
	if Player_GetYieldStored(player, YieldTypes.YIELD_CULTURE) >= Player_GetYieldNeeded(player, YieldTypes.YIELD_CULTURE) then
		justLooking = false;
	end
			...
			if not player:IsPolicyBranchUnlocked( i ) then
				...
				if (not player:CanUnlockPolicyBranch(i)) then
					...
					-- Don't have enough Culture Yet
						...
						strToolTip = strToolTip .. " " .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_CANNOT_UNLOCK_CULTURE", Player_GetYieldNeeded(player, YieldTypes.YIELD_CULTURE));
				...
				-- Can adopt this branch right now
					...
					strToolTip = strToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_POLICY_BRANCH_UNLOCK_SPEND", Player_GetYieldNeeded(player, YieldTypes.YIELD_CULTURE));
