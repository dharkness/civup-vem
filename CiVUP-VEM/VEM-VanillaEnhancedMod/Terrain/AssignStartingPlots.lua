...
function AssignStartingPlots:MeasureStartPlacementFertilityOfPlot(x, y, checkForCoastalLand)
	...
	--
	--[[ Mountain, Oasis, FloodPlain tiles = -1, 6, 7 and do not count anything else.
	     ...
	     We want players who start in Plains to have the least room to expand. ]]--
...
function AssignStartingPlots:CanBeGibraltar(x, y)
	...
	local areaWeights = GetAreaWeights(plot, 2, 4)
	if (areaWeights.PLOT_HILLS + areaWeights.FEATURE_JUNGLE <= 0.1
		or not IsBetween(0.1, areaWeights.PLOT_LAND + areaWeights.PLOT_HILLS, 0.6)
...
function AssignStartingPlots:CanBeReef(x, y)
	...
	local areaWeights = GetAreaWeights(plot, 2, 5)
...
function AssignStartingPlots:CanBeKrakatoa(x, y)
	...
	areaWeights = GetAreaWeights(plot, 0, 1)
...
function AssignStartingPlots:BuffIslands()
	...
	for index, plot in Plots(Shuffle) do
		...
		if ((plotType == PlotTypes.PLOT_HILLS or plotType == PlotTypes.PLOT_LAND )
			...
			local resNum = Map.Rand(3, "BuffIslands")
			if resNum == 0 then
				resID  = self.stone_ID
				resNum = 1
			else
...
function AssignStartingPlots:CalculateStrategicPlotWeights()
	...
	for x = 0, iW - 1 do
		for y = 0, iH - 1 do
			...
			local plotID		= y * iW + x + 1
			local terrainType	= plot:GetTerrainType()
			for _, adjPlot in pairs(GetPlotsInCircle(plot, 0, CiVUP.RESOURCE_PLOT_BLUR_DISTANCE)) do
				local adjX			= adjPlot:GetX()
				local adjY			= adjPlot:GetY()
				...
				for _, resID in pairs(resIDs) do
					...
					if resID == self.oil_ID and plot:GetTerrainType() == TerrainTypes.TERRAIN_COAST and not plot:IsLake() then
						if adjPlotType ~= PlotTypes.PLOT_OCEAN then
							...
							--self.plotResInfo[resID][plotID].weightAvg = self.plotResInfo[resID][plotID].weightAvg * (1 - 1/(distance + 1))
						end
						self.plotResInfo[resID][plotID].weightAvg = self.plotResInfo[resID][plotID].weightAvg + adjWeight / (distance + 1)
						self.plotResInfo[resID][plotID].numPlots = self.plotResInfo[resID][plotID].numPlots + 1 / (distance + 1)
					end
		...
		for region_loop_x = 0, iWidth - 1 do
			...
			for _, resID in pairs(resIDs) do
				if self.plotResInfo[resID][plotID].weightAvg > 0 then
					...
					elseif resID == self.oil_ID and plot:GetTerrainType() == TerrainTypes.TERRAIN_COAST and plot:IsAdjacentToLand() and not plot:IsLake() then
						table.insert(plotList[resID], plotID)
					end
				end
			end
...
function AssignStartingPlots:GetStrategicDistributionForPlots(resID, plotList)
	...
	return Round(resAmount), maxDepositSize
end
------------------------------------------------------------------------------
